# Distributed EVPN Symmetric Routing With Multi-Homing Inventory

This repository holds the best-practice configuration for deployment of the distributed inter-subnet routing using [EVPN Symmetric](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-51/Network-Virtualization/Ethernet-Virtual-Private-Network-EVPN/Inter-subnet-Routing/#symmetric-routing) control plane with SVI as gateways on the leaf (ToR) switches. In this demo, we demonstrate the all-active server redundant environment with [EVPN Multihoming](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-51/Network-Virtualization/Ethernet-Virtual-Private-Network-EVPN/EVPN-Multihoming/) configuration to present EVPN fabric with **standardized L2 redundancy** on the ToR switches. We use the [NVUE Object Model](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-51/System-Configuration/NVIDIA-User-Experience-NVUE/NVUE-Object-Model/) and [NVUE CLI](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-51/System-Configuration/NVIDIA-User-Experience-NVUE/NVUE-CLI/) to accomplish that.
...
```bash
ubuntu@server:~/cumulus_ansible_modules$ ansible pod1 -i inventories/evpn_mh/hosts -m ping
fw1 | SUCCESS => {
    "ansible_facts": {
...
```bash
ubuntu@server:~/cumulus_ansible_modules$ ansible-playbook playbooks/nvue.yml -i inventories/evpn_mh/hosts --diff
```
...
```bash
ubuntu@server:~/cumulus_ansible_modules$ cd inventories/evpn_mh/config
```
...
```bash
ubuntu@oob-mgmt-server:~$ ssh ubuntu@server03
