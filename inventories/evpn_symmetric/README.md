# Distributed EVPN Symmetric Routing Inventory
...
```bash
cumulus@server:mgmt:~$ netq check bgp
...
```
```bash
cumulus@server:mgmt:~$ netq check mlag
...
```bash
cumulus@server:mgmt:~$ netq check 
...
```bash
cumulus@server:mgmt:~$ netq trace 10.1.20.105 from 10.1.10.101 detail
Number of Paths: 16
...
```bash
cumulus@server:mgmt:~$ netq check mtu
...
Overlay network monitoring:
```bash
cumulus@server:mgmt:~$ netq show vxlan
