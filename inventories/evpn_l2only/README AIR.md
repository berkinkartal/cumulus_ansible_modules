...
```bash
ubuntu@oob-mgmt-server:~/cumulus_ansible_modules$ ansible pod1 -i inventories/evpn_l2only/hosts -m ping
fw1 | SUCCESS => {
    "ansible_facts": {
...
```bash
ubuntu@oob-mgmt-server:~/cumulus_ansible_modules$ ansible-playbook playbooks/nvue.yml -i inventories/evpn_l2only/hosts
```
...
```bash
ubuntu@oob-mgmt-server:~/cumulus_ansible_modules$ cd inventories/evpn_l2only/config
```
